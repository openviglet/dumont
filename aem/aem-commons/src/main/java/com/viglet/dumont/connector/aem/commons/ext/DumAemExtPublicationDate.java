...
@Slf4j
public class DumAemExtPublicationDate implements DumAemExtAttributeInterface {
	@Override
	public TurMultiValue consume(DumAemTargetAttr dumAemTargetAttr,
		...
		log.debug("Executing DumAemExtPublicationDate");
		return Optional.ofNullable(aemObject.getPublicationDate())
				.map(publicationDate -> TurMultiValue.singleItem(publicationDate.getTime()))
				.orElse(null);
	}
}
