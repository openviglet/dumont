...
@Slf4j
@Component
public class DumAemReactiveUtils {

    private final DumAemReactiveHttpService reactiveHttpService;

    public DumAemReactiveUtils(DumAemReactiveHttpService reactiveHttpService) {
     ...
     * @param dumAemSourceContext the source context
     * @return Mono containing JSONObject or empty if not found/invalid
     */
    public Mono<JSONObject> getInfinityJsonReactive(String url, DumAemConfiguration dumAemSourceContext) {
        String infinityJsonUrl = String.format(url.endsWith(JSON) ? "%s%s" : "%s%s.infinity.json",
                dumAemSourceContext.getUrl(), url);

        log.debug("Getting infinity JSON reactively for: {}", infinityJsonUrl);

        return reactiveHttpService.fetchResponseBodyReactive(infinityJsonUrl, dumAemSourceContext)
                    ...
                    try {
                        if (isResponseBodyJSONArray(responseBody) && !url.endsWith(JSON)) {
                            ...
                            if (!jsonArray.isEmpty()) {
                                ...
                                return getInfinityJsonReactive(firstItem, dumAemSourceContext);
                            }
