package com.viglet.dumont.connector.plugin.webcrawler.persistence.repository;

import com.viglet.dumont.connector.plugin.webcrawler.persistence.model.DumWCAttributeMapping;
import com.viglet.dumont.connector.plugin.webcrawler.persistence.model.DumWCSource;
import org.springframework.data.jpa.repository.JpaRepository;

import java.util.List;
import java.util.Optional;

public interface DumWCAttributeMappingRepository extends JpaRepository<DumWCAttributeMapping, String> {
    Optional<List<DumWCAttributeMapping>> findByDumWCSource(DumWCSource dumWCSource);
}
