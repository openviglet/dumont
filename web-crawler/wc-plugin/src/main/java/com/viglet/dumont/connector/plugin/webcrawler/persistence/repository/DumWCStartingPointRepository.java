package com.viglet.dumont.connector.plugin.webcrawler.persistence.repository;

import com.viglet.dumont.connector.plugin.webcrawler.persistence.model.DumWCSource;
import com.viglet.dumont.connector.plugin.webcrawler.persistence.model.DumWCStartingPoint;
import org.springframework.data.jpa.repository.JpaRepository;

import java.util.List;
import java.util.Optional;

public interface DumWCStartingPointRepository extends JpaRepository<DumWCStartingPoint, String> {
    Optional<List<DumWCStartingPoint>> findByDumWCSource(DumWCSource dumWCSource);
}
