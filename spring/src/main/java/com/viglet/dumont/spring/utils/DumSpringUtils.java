 ...
 */
@Slf4j
public class DumSpringUtils {

    private DumSpringUtils() {
    ...
    public static File extractZipFile(MultipartFile file) {
        ...
        File extractFolder = new File(
                randomTempFileOrDirectory());
        try {
            DumCommonsUtils.unZipIt(zipFile, extractFolder);
    ...
    private static String randomTempFileOrDirectory() {
        return DumCommonsUtils.getTempDirectory().getAbsolutePath().concat(File.separator + "imp_" + UUID.randomUUID());
    }
}
