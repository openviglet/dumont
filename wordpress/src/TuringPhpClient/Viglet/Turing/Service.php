...
class Viglet_Dumont_Service
{
     ...
     *
     * @var Viglet_Dumont_HttpTransport_Interface
     */
     ...
     * @param string $port
     * @param string $path
     * @param Viglet_Dumont_HttpTransport_Interface $httpTransport
     ...
     * @return Viglet_Dumont_Response
     *
     * @throws Viglet_Dumont_HttpTransportException If a non 200 response status is returned
     */
    protected function _sendGet($url, $timeout = FALSE)
    {
        ...
        $solrResponse = new Viglet_Dumont_Response($httpResponse, $this->_createDocuments, $this->_collapseSingleValueArrays);

        if ($solrResponse->getHttpStatus() != 200) {
            throw new Viglet_Dumont_HttpTransportException($solrResponse);
        }
     ...
     *
     * @throws Viglet_Dumont_InvalidArgumentException If the host parameter is empty
     */
    public function setSiteName($siteName)
    {
        // Use the provided host or use the default
        if (empty($siteName)) {
            throw new Viglet_Dumont_InvalidArgumentException('Site Name parameter is empty');
        } else {
    ...
    public function setHost($host)
    {
        // Use the provided host or use the default
        if (empty($host)) {
            throw new Viglet_Dumont_InvalidArgumentException('Host parameter is empty');
        } else {
            $this->_host = $host;
        }
     ...
     * @param integer $port
     *
     * @throws Viglet_Dumont_InvalidArgumentException If the port parameter is empty
     */
    public function setPort($port)
    {
        ...
        if ($port <= 0) {
            throw new Viglet_Dumont_InvalidArgumentException('Port is not a valid port number');
        } else {
            $this->_port = $port;
        }
    ...
    {
        if (empty($protocol)) {
            throw new Viglet_Dumont_InvalidArgumentException('Protocl parameter is empty');
        } else {
     ...
     *
     * @return Viglet_Dumont_HttpTransport_Interface
     */
    public function getHttpTransport()
    {
        ...
        if ($this->_httpTransport === false) {
            ...
            $this->_httpTransport = new Viglet_Dumont_HttpTransport_Curl();
        }
     ...
     *
     * @param
     *            Viglet_Dumont_HttpTransport_Interface
     */
    public function setHttpTransport(Viglet_Dumont_HttpTransport_Interface $httpTransport)
    {
        $this->_httpTransport = $httpTransport;
    }

    /**
     * Create an XML fragment from a {@link Viglet_Dumont_Document} instance appropriate for use inside a Solr add call
     *
     * @return string
     */
    protected function _documentToXmlFragment(Viglet_Dumont_Document $document)
    {
     ...
     *
     * @param Viglet_Dumont_Document $document
     ...
     * @return Viglet_Dumont_Response
     *
     * @throws Viglet_Dumont_HttpTransportException If an error occurs during the service call
     */
    ...
    public function addDocuments($documents, $allowDups = false, $overwritePending = true, $overwriteCommitted = true, $commitWithin = 0)
    {
        ...
        foreach ($documents as $document) {
            $rawPost = "";
            if ($document instanceof Viglet_Dumont_Document) {
                $rawPost .= $this->_documentToXmlFragment($document);
            }
