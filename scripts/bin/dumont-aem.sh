#!/usr/bin/env bash
# --------------------------------------------------------------------------
# Start Dumont Connector with the AEM plugin.
#
# Usage:  ./bin/dumont-aem.sh [JVM options...] [-- Spring options...]
#
# Examples:
#   ./bin/dumont-aem.sh
#   ./bin/dumont-aem.sh -Xmx1g
#   ./bin/dumont-aem.sh -Xmx1g -- --server.port=30131
# --------------------------------------------------------------------------
set -euo pipefail
...
if [ ! -d "$LIBS_DIR" ]; then
  echo "Error: AEM plugin directory not found: $LIBS_DIR" >&2
  exit 1
fi
